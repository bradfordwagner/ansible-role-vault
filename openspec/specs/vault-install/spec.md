# vault-install Specification

## Purpose
Installs HashiCorp Vault (Community or Enterprise) onto a host: resolving the requested version, downloading and checksum-verifying the release archive, unarchiving it into a versioned install directory, and symlinking the binary into a link directory — for either system-wide or per-user (local) installs.
